...
class WorkerExtension:
    ...
    def load_self_weights_from_disk(self, filepath):
        """Load weights from a checkpoint saved by save_self_weights_to_disk."""
        state_dict = torch.load(filepath, map_location="cpu")

        for name, p in self.model_runner.model.named_parameters():
            if name in state_dict:
                p.data.copy_(state_dict[name].to(p.device, p.dtype))
            else:
    ...
    def store_base_weights(self):
        """Store a CPU copy of current weights as the base for norm tracking."""
        self._base_weights = {}
        for name, p in self.model_runner.model.named_parameters():
            self._base_weights[name] = p.detach().cpu().float()
        return True

    def compute_delta_norms(self):
        """Compute per-parameter ||theta_t - theta_0||^2 and return as dict."""
        result = {}
        for name, p in self.model_runner.model.named_parameters():
            delta = p.detach().cpu().float() - self._base_weights[name]
            norm_sq = float(torch.sum(delta * delta))
            result[name] = {
                "delta_norm_sq": norm_sq,
                "shape": list(p.shape),
                "numel": p.numel(),
            }
            del delta
