...
from .vllm_compat import is_requests_available, is_vllm_ascend_available, is_vllm_available
...
class VLLMClient:
    ...
    def generate(
        ...
        max_len = max(len(p) for p in prompts) if prompts else 0
        print(f"[VLLM_CLIENT] generate: n_prompts={len(prompts)} max_len={max_len} total_chars={total_chars}", flush=True)
        if max_len > 50000:
            print(f"[VLLM_CLIENT] WARNING: Very long prompt detected!", flush=True)
        ...
        response = self.session.post(
            ...
            timeout=600,  # 10 minute timeout - fail fast instead of hanging
        )
        if response.status_code == 200:
