...
from .cli_utils import TrlParser
...
def llm_worker(
    ...
    llm = LLM(
        ...
        worker_extension_cls="es_vs_grpo.rollout.vllm_server.WeightSyncWorkerExtension",
...
def main(script_args: ScriptArguments):
    ...
    async def generate(request: GenerateRequest):
        ...
        max_len = max(len(p) for p in request.prompts) if request.prompts else 0
        total_chars = sum(len(p) for p in request.prompts)
        print(f"[VLLM_SERVE] Received: n_prompts={n_prompts} max_len={max_len} total_chars={total_chars}", flush=True)
